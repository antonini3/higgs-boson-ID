...
def evaluate_lenet5(learning_rate=0.1, n_epochs=200,
    ...
    x_train, x_valid, y_train, y_valid = train_test_split(x_total, y_total, test_size=TEST_SET_RATIO, random_state=42)
    max_x = max(x_train.max(), x_train.max())
    x_train *= 1.0 / max_x
    x_valid *= 1.0 / max_x
    y_train = boolean_y(y_train)
    y_valid = boolean_y(y_test)


    # compute number of minibatches for training, validation and testing
    n_train_batches = x_train.get_value(borrow=True).shape[0]
    n_valid_batches = y_train.get_value(borrow=True).shape[0]
