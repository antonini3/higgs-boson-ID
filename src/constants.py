
NUM_PIXELS =  25 

IMG_DIMENSION = NUM_PIXELS ** 2

TEST_SET_RATIO = 0.25

VALTEST_SET_RATIO = 0.35

VAL_SET_RATIO = 0.5

NUM_CLASSES = 2

PADDED_NUM_PIXELS = 32

PADDED_IMAGE_DIMENSION = PADDED_NUM_PIXELS ** 2

COLOURMAP_COLOUR = 'Blues'


HIGGS_FILE_NAME = "../data/ColorflowData_Higgs_50k.txt"

NOT_HIGGS_FILE_NAME = "../data/ColorflowData_NotHiggs_50k.txt"

HIGGS_FILE_NAME_W_PULL = "../data/ColorflowData_Higgs_25x25_withpull.txt"

NOT_HIGGS_FILE_NAME_W_PULL = "../data/ColorflowData_notHiggs_25x25_withpull.txt"

FINE_HIGGS_FILE_NAME_W_PULL = "../data/ColorflowData_Higgs_fine_50k.txt"

FINE_NOT_HIGGS_FILE_NAME_W_PULL = "../data/ColorflowData_NotHiggs_fine_50k.txt"

